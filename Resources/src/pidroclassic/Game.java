...
import pidroclassic.deck.Deck;
import pidroclassic.deck.card.Card;
import pidroclassic.deck.card.Suit;
import pidroclassic.exception.*;
...
public class Game implements Serializable {
    ...
    public void join(String playerName) throws GameIsFullException {
        try {
            joinTeam(TeamName.WE, playerName);
        }
        catch (TeamIsFullException e) {
            try {
                joinTeam(TeamName.THEY, playerName);
                }
            catch (TeamIsFullException e2) {
                throw new GameIsFullException();
            }
        }
    }

    public void joinTeam(TeamName team, String playerName) throws TeamIsFullException {
    ...
    public boolean biddingIsDone(){return bidCount == 4;}
     ...
     */
    public void findInitDealer() throws GameNotReadyException{
        if(!isReady())
        ...
        refreshDeck(); //Refresh the pidroclassic.deck
        System.out.printf("%s is the first dealer.\n", dealer);
    }

    public void splitDeck(int amount) {
        this.deck.poke(amount);
    }

    /**
     * Performs the first deal of 3*3 cards per player from a newly shuffled pidroclassic.deck.
     */
     ...
     */
    public void bidCurrentPlayerAndNext(int bid) throws IllegalBidException, LastMustBidException {
        //Illegal bid
     ...
     */
    public void popIrrelevantCards() {
        for(Team team : teams)
            for(int i = 0; i <= 1; i++)
    ...
    public Player getWinningBidPlayer() {
    ...
    public void printPlayerCards(){
        ...
        for(int i = 0; i < 4; i++) {
            ...
            System.out.printf("%s cards: %s%n", temp, temp.cardsAsString());
        }
